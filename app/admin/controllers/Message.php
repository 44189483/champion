...
class Message extends CI_Controller{
	...
	public function index(){
	    ...
	    $query = $this->db->query("SELECT * FROM message {$where}");
	    $result['total'] = $query->num_rows();

	    $query = $this->db->query("SELECT * FROM message {$where} ORDER BY id DESC LIMIT {$offset},{$config['per_page']}");
	    $result['list'] = $query->result();
	   
	    $config['base_url'] = site_url('message?').$search;
		...
		$this->load->view('message/index.html',$data);
	}
	...
	public function view($id=null){
	    ...
	    $data = $query->row();
		$this->load->view('message/view.html',$data);
	...
	public function del($id = null){
		...
		if($pid){//多删
			...
			$bool = $this->db->query("DELETE FROM message WHERE id IN(".implode(',', $pid).")"); 

		}else if (!empty($gid)) {//单删

			$bool = $this->db->delete('message', array('id' => $gid));

		}
		
		if($bool) {
            jump('操作成功',site_url('message/index'));
        }else{
            jump('操作失败');
        }
