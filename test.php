<?php
include 'sealtalk-sdk/API/rongcloud.php';
$appKey = 'mgb7ka1nm4esg';
$appSecret = 'JiKsJNCCobSkcL';
$jsonPath = "sealtalk-sdk/API/jsonsource/";
$RongCloud = new RongCloud($appKey,$appSecret);

$userid = 3;
$username = '都市浪少';
$head = 'http://www.rongcloud.cn/images/logo.png';


	echo ("<br/>***************** user **************<br/>");
	// 获取 Token 方法
	$result = $RongCloud->user()->getToken($userid, $username, $head);
	echo "getToken    ";
	print_r($result);
	echo "<br/>";
	
	// 刷新用户信息方法
	$result = $RongCloud->user()->refresh($userid, $username, $head);
	echo "refresh    ";
	print_r($result);
	echo "<br/>";
	...
	$result = $RongCloud->user()->addBlacklist($userid, 'userId2');
	echo "addBlacklist    ";
	print_r($result);
	echo "<br/>";
	...
	$result = $RongCloud->user()->removeBlacklist($userid, 'userId2');
	...
	$result = $RongCloud->message()->publishPrivate($userid, ["userId2","userid3","userId4"], 'RC:VcMsg',"{\"content\":\"hello\",\"extra\":\"helloExtra\",\"duration\":20}", 'thisisapush', '{\"pushData\":\"hello\"}', '4', '0', '0', '0', '0');
	echo "publishPrivate    ";
	print_r($result);
	echo "<br/>";
	...
	$result = $RongCloud->message()->PublishSystem($userid, ["userId2","userid3","userId4"], 'RC:TxtMsg',"{\"content\":\"hello\",\"extra\":\"helloExtra\"}", 'thisisapush', '{\"pushData\":\"hello\"}', '0', '0');
	echo "PublishSystem    ";
	print_r($result);
	echo "<br/>";
	...
	$result = $RongCloud->message()->publishDiscussion($userid, 'discussionId1', 'RC:TxtMsg',"{\"content\":\"hello\",\"extra\":\"helloExtra\"}", 'thisisapush', '{\"pushData\":\"hello\"}', '1', '1', '0');
	echo "publishDiscussion    ";
	print_r($result);
	echo "<br/>";
	
	// 发送聊天室消息方法（一个用户向聊天室发送消息，单条消息最大 128k。每秒钟限 100 次。）
	$result = $RongCloud->message()->publishChatroom($userid, ["ChatroomId1","ChatroomId2","ChatroomId3"], 'RC:TxtMsg',"{\"content\":\"hello\",\"extra\":\"helloExtra\"}");
	...
	$result = $RongCloud->message()->broadcast($userid, 'RC:TxtMsg',"{\"content\":\"哈哈\",\"extra\":\"hello ex\"}", 'thisisapush', '{\"pushData\":\"hello\"}', 'iOS');
	echo "broadcast    ";
	print_r($result);
	echo "<br/>";
	...
	$result = $RongCloud->group()->sync($userid, $groupInfo);
	echo "sync    ";
	print_r($result);
	echo "<br/>";
	...
	$result = $RongCloud->group()->addGagUser($userid, 'groupId1', '1');
	echo "addGagUser    ";
	print_r($result);
	echo "<br/>";
	...
	$result = $RongCloud->group()->dismiss($userid, 'groupId1');
	echo "dismiss    ";
	print_r($result);
	echo "<br/>";
	...
	$result = $RongCloud->chatroom()->addGagUser($userid, 'chatroomId1', '1');
	echo "addGagUser    ";
	print_r($result);
	echo "<br/>";
	...
	// 移除禁言聊天室成员方法
	$result = $RongCloud->chatroom()->rollbackGagUser($userid, 'chatroomId1');
	echo "rollbackGagUser    ";
	print_r($result);
	echo "<br/>";
	
	// 添加封禁聊天室成员方法
	$result = $RongCloud->chatroom()->addBlockUser($userid, 'chatroomId1', '1');
	echo "addBlockUser    ";
	print_r($result);
	echo "<br/>";
	...
	// 移除封禁聊天室成员方法
	$result = $RongCloud->chatroom()->rollbackBlockUser($userid, 'chatroomId1');
