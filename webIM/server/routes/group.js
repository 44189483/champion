...
sendGroupNotification = function(userId, groupId, operation, data) {
  ...
  console.log('Sending GroupNotificationMessage:', encodedGroupId);
...
router.post('/add_to_address', function(req, res, next) {
  var currentUserId, groupId, show, timestamp;
  groupId = req.body.groupId;
  show = req.body.show;
  currentUserId = Session.getCurrentUserId(req);
  timestamp = Date.now();
...
router.get('/:count/head_info', function(req, res, next) {
  var count, currentUserId, sql;
  count = req.params.count;
  currentUserId = Session.getCurrentUserId(req);
  sql = ['select t.groupId, u.id, u.phone, u.nickname, u.portraitUri, u.email from group_members t ', 'inner join users u on t.memberId = u.id ', 'where ' + count + ' > (select count(*) from group_members where groupId = t.groupId and createdAt > t.createdAt) ', 'and groupId in (select groupId from group_members where memberId = "' + currentUserId + '") order by t.groupId'].join('');
  return sequelize.query(sql, {
    type: sequelize.QueryTypes.SELECT
  }).then(function(heads) {
    var results;
    results = Utility.encodeResults(heads, ['id', 'groupId']);
  ...
  return Cache.get("group_" + groupId).then(function(group) {
    if (group) {
      ...
      return Group.findById(groupId, {
        attributes: ['id', 'name', 'portraitUri', 'memberCount', 'maxMemberCount', 'creatorId', 'type', 'bulletin', 'deletedAt'],
        paranoid: false
      }).then(function(group) {
        var results;
        if (!group) {
...
router.get('/info/:id/members', function(req, res, next) {
  var currentUserId, groupId;
  groupId = req.params.id;
  groupId = Utility.decodeIds(groupId);
  currentUserId = Session.getCurrentUserId(req);
  return Cache.get("group_members_" + groupId).then(function(groupMembers) {
    ...
    if (groupMembers) {
      isInGroup = groupMembers.some(function(groupMember) {
        var memberId;
        memberId = Utility.decodeIds(groupMember.id);
      ...
      sql = ['select ', 't.displayName, t.role, t.createdAt, t.updatedAt,', 't1.id, t1.region,t1.phone, t1.tel, t1.email, t1.nickname,', 't1.portraitUri, t2.deptId, t3.deptName,', 't4.dutyName, t2.managerId, ', 't5.nickname as managerName,', 't5.portraitUri as managerPortrait, t5.phone as managerPhone, if(t6.targetId,1,0) as star ', 'from group_members t ', 'left join users t1 on t.memberId = t1.id ', 'left join dept_members t2 on t1.id = t2.userId ', 'left join departments t3 on t2.deptId = t3.id ', 'left join duties t4 on t2.dutyId = t4.id ', 'left join users t5 on t2.managerId = t5.id ', 'left join user_relations t6 on t.memberId = t6.targetId and t6.userId =' + currentUserId, ' where t.groupId = ' + groupId].join('');
      return sequelize.query(sql, {
        type: sequelize.QueryTypes.SELECT
      }).then(function(members) {
        var results;
        ...
        results = Utility.encodeResults(members, ['id', 'deptId', 'managerId']);
        Cache.set("group_members_" + groupId, results);
        return res.send(new APIResult(200, results));
      });
    }
