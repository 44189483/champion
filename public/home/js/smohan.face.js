...
$(function() {
	$.fn.smohanfacebox = function(options) {
	    ...
	    for(i=0;i<60;i++){  //通过循环创建60个表情，可扩展
		 faceimg+='<li><a href="javascript:void(0)"><img src="/public/home/images/face/'+(i+1)+'.gif" face="<emt>'+(i+1)+'</emt>"/></a></li>';
		 };
		...
		//插入表情
		$facepic.click(function(){//.die()
		     $('#SmohanFaceBox').hide(360);
			 //$("#"+options.textid).focus();
			 //$("#"+options.textid).val($("#"+options.textid).val()+$(this).attr("face"));
			 $("#"+options.textid).insertContent($(this).attr("face"));//.die()
			 $btn.removeClass('in');
			});			
		//关闭表情层
